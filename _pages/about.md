---
permalink: /
title: false
author_profile: true
redirect_from:
  - /about/
  - /about.html
---

<h2 id="about">Personal Information</h2>

**Affiliation:** School of Data Science and Statistics, Anhui University

**Position:** Lecturer

**Degree:** Ph.D.

**Email:** [zjsong@ahu.edu.cn](mailto:zjsong@ahu.edu.cn)

**Biography:** Zijie Song is currently a Lecturer at the School of Data Science and Statistics, Anhui University. He received his Ph.D. degree in Computer Science and Technology from Hefei University of Technology in 2025 under the supervision of Prof. Richang Hong. His research interests include multimedia technology, cross-modal understanding, vision-language learning, and their applications.

He has participated in several research projects supported by the National Key Research and Development Program of China and the National Natural Science Foundation of China. He has published more than ten papers in international journals and conferences, including IEEE Transactions on Multimedia, NeurIPS, and IJCAI. He also serves as a reviewer for international journals and conferences, including IEEE Transactions on Multimedia, ACM Transactions on Multimedia Computing, Communications, and Applications, and ICME. He is a member of the Multimedia Technical Committee of the China Society of Image and Graphics.

<h2 id="news" class="news-title">News</h2>

- The paper “Image Captioning via Compact Bidirectional Architecture” was published in IEEE Transactions on Multimedia, 2026. (CCF-A; CAS Q1 Top Journal)

- The paper “Enhanced Keyframe Selection for Long-Video Question Answering via Large Language Model-Generated Pseudo-Labels” was published in Journal of Computer Research and Development, 2026, 63(6): 1563–1573. (CCF Chinese T1; Chinese Core Journal)

- Zijie Song received the Best Poster Award at ChinaMM 2025.

<h2 id="research">Research Interests</h2>

Multimedia technology, cross-modal understanding, and vision-language learning.  
Image captioning, video question answering, text-video retrieval, and image clustering.

<h2 id="education">Education</h2>

<table class="simple-table education-table">
  <tbody>
    <tr>
      <td>2022.08—2025.08</td>
      <td>Ph.D. in Computer Science and Technology, Hefei University of Technology</td>
    </tr>
    <tr>
      <td>2019.09—2022.05</td>
      <td>M.Eng. in Electronics and Communication Engineering, Hefei University of Technology</td>
    </tr>
    <tr>
      <td>2015.09—2019.06</td>
      <td>B.S. in Information and Computing Science, North China Electric Power University</td>
    </tr>
  </tbody>
</table>

<h2 id="projects">Research Projects</h2>

<table class="simple-table">
  <tbody>
    <tr>
      <td style="width: 25%; white-space: nowrap; text-align: center;">2026.06—2027.06</td>
      <td>University–Industry Collaborative Research Project (Principal Investigator)</td>
    </tr>
    <tr>
      <td style="width: 25%; white-space: nowrap; text-align: center;">2022.01—2025.12</td>
      <td>General Program of the National Natural Science Foundation of China (Key Project Member)</td>
    </tr>
  </tbody>
</table>

<h2 id="publications">Selected Publications</h2>

1. <strong>Z. Song</strong>, Z. Hu, Y. Zhou, Y. Zhao, R. Hong, and M. Wang. Embedded Heterogeneous Attention Transformer for Cross-lingual Image Captioning. IEEE Transactions on Multimedia, 2024, 26: 9008–9020.

2. <strong>Z. Song</strong>, Z. Hu, Y. Ma, J. Li, and R. Hong. Video Flow as Time Series: Discovering Temporal Consistency and Variability for VideoQA. IEEE International Conference on Multimedia and Expo (ICME), 2025.

3. J. Xiao, <strong>Z. Song</strong>, J. Hu, H. Cheng, Z. Hu, J. Li, and R. Hong. Rebalancing Contrastive Alignment with Learnable Semantic Gaps in Text-Video Retrieval. Advances in Neural Information Processing Systems (NeurIPS), 2025.

4. <strong>Z. Song</strong>, Z. Hu, and R. Hong. Efficient and Self-adaptive Rationale Knowledge Base for Visual Commonsense Reasoning. Multimedia Systems, 2023, 29(5): 3017–3026.

5. <strong>Z. Song</strong>, Z. Hu, and R. Hong. Grid Feature Jigsaw for Self-supervised Image Clustering. International Joint Conference on Neural Networks (IJCNN), 2023.

For more research work and academic service, please refer to:

Google Scholar: [https://scholar.google.com/citations?user=99X3AYkAAAAJ](https://scholar.google.com/citations?user=99X3AYkAAAAJ)  
ORCID: [https://orcid.org/0000-0002-1262-764X](https://orcid.org/0000-0002-1262-764X)

<h2 id="teaching">Teaching</h2>

Undergraduate Courses: Java Technology and Applications; Big Data System Development.

<h2 id="service">Awards and Academic Service</h2>

- Best Poster Award at ChinaMM 2025
- Best Paper Nomination
- Member of the Multimedia Technical Committee, China Society of Image and Graphics
- Reviewer for IEEE Transactions on Multimedia
- Reviewer for ACM Transactions on Multimedia Computing, Communications, and Applications
- Reviewer for ICME and other international conferences

<h2 id="admissions">Admissions</h2>

1. Students interested in multimedia technology, cross-modal understanding, vision-language learning, and related research topics are welcome to join the group.

2. Applicants are expected to be outgoing, responsible, and able to communicate and collaborate effectively with others.

3. A strong mathematical foundation and good English academic reading skills are preferred.

4. Applicants should have solid programming skills and be familiar with at least one mainstream programming language, such as Python, C++, or Java.

5. Applicants should be self-motivated, willing to learn, and able to devote sufficient time and effort to research.
